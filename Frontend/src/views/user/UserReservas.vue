        ...
        <div
            ...
            <div class="px-4 pt-8 pb-5">
                ...
                <p class="text-slate-400 text-sm font-medium mt-2">
                    Gestioná tus turnos y seguí tu historial.
                </p>
            </div>
                            ...
                            <div class="absolute top-7 right-7 z-50">
                                ...
                                <transition name="fade">
                                    <div v-if="showMenuId === t.id"
                                        ...
                                        <button @click.stop="compartirWhatsApp(t)"
                                            ...
                                            <i class="fab fa-whatsapp text-emerald-500 w-4 text-sm"></i>
                                            Compartir
                                        </button>
                                        <button @click.stop="agregarAlCalendario(t)"
                                            class="w-full px-5 py-3.5 text-left text-[11px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-3 transition-colors">
                                            <i class="fas fa-calendar-plus text-slate-400 w-4"></i>
                                            Agregar al calendario
                                        </button>
                                        <button @click.stop="abrirComoLlegar(t)"
                                            class="w-full px-5 py-3.5 text-left text-[11px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-3 transition-colors">
                                            <i class="fas fa-location-arrow text-slate-400 w-4"></i>
                                            Como llego
                                        </button>
                                        <button @click.stop="abrirTelefono(t)"
                                            class="w-full px-5 py-3.5 text-left text-[11px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-3 transition-colors">
...
export default {
    ...
    computed: {
        porConfirmar() {
            return this.turnos.filter(t => t.estado && t.estado.toLowerCase().includes('pendiente'));
        },
        confirmadas() {
            return this.turnos.filter(t => t.estado && t.estado.toLowerCase().includes('confirmado'));
        },
        turnoSeleccionado() {
    ...
    methods: {
        ...
        async confirmCancel() {
            ...
            try {
                ...
                this.cancellingId = null;
                this.showToast('No se pudo cancelar el turno. Verificá tu conexión o intentá nuevamente.', 'error');
            } finally {
                stopLoader();
            }
        },
        ...
        formatearFechaLegible(fechaStr, fechaIso) {
            let date = null;
            if (fechaIso) {
            ...
            if (!date || isNaN(date.getTime())) {
                return fechaStr || 'Fecha a confirmar';
            }
            ...
            return `${capitalize(diaSemana)} ${diaNumero} de ${mes}`;
        },

        obtenerCantidadJugadores(turno) {
            const texto = `${turno.deporte || ''} ${turno.cancha || ''} ${turno.complejo || ''}`.toLowerCase();

            if (texto.includes('padel') || texto.includes('pádel')) {
                return 4;
            }
            if (texto.includes('tenis') || texto.includes('tennis')) {
                return 4;
            }
            if (texto.includes('futbol 11') || texto.includes('fútbol 11') || texto.includes('futbol11') || texto.includes('f11')) {
                return 22;
            }
            if (texto.includes('futbol 9') || texto.includes('fútbol 9') || texto.includes('futbol9') || texto.includes('f9')) {
                return 18;
            }
            if (texto.includes('futbol 8') || texto.includes('fútbol 8') || texto.includes('futbol8') || texto.includes('f8')) {
                return 16;
            }
            if (texto.includes('futbol 7') || texto.includes('fútbol 7') || texto.includes('futbol7') || texto.includes('f7')) {
                return 14;
            }
            if (texto.includes('futbol 5') || texto.includes('fútbol 5') || texto.includes('futbol5') || texto.includes('f5')) {
                return 10;
            }
            if (texto.includes('basquet') || texto.includes('básquet')) {
                return 10;
            }
            return 10;
        },

        compartirWhatsApp(turno) {
            this.showMenuId = null;

            const nombreCancha = (turno.cancha && turno.complejo && !turno.cancha.toLowerCase().includes(turno.complejo.toLowerCase()))
                ? `${turno.complejo} - ${turno.cancha}`
                : (turno.complejo || turno.cancha || 'Cancha');

            const fechaFormateada = this.formatearFechaLegible(turno.fecha, turno.fechaIso);
            const horaStr = `${turno.hora} hs`;
            const cantidadJugadores = this.obtenerCantidadJugadores(turno);
            const precioTotal = Number(turno.precio) || 0;
            const precioPorPersona = cantidadJugadores > 0 ? Math.round(precioTotal / cantidadJugadores) : precioTotal;
            const medioPago = turno.medioPago ? turno.medioPago.toLowerCase() : 'efectivo';
            const direccion = turno.direccion || 'Av. Santa Fe 1234';
            const link = `${window.location.origin}/#/reservas`;

            const texto = `⚽ ¡Partido reservado!
...
💰 $${precioTotal.toLocaleString('es-AR')} la cancha (${medioPago})
💵 $${precioPorPersona.toLocaleString('es-AR')} por persona
...
🔗 Ver reserva: ${link}`;
        ...
        agregarAlCalendario(turno) {
            ...
            try {
                ...
                if (turno.fechaIso) {
                    datePart = turno.fechaIso.split('T')[0].replace(/-/g, '');
                } else if (turno.fecha && turno.fecha.includes('/')) {
                    const [d, m, y] = turno.fecha.split('/');
                    datePart = `${y}${m.padStart(2, '0')}${d.padStart(2, '0')}`;
                }

                const horaInicioClean = (turno.hora || '20:00').replace(':', '') + '00';
                const horaFinClean = (turno.horaFin || '21:00').replace(':', '') + '00';

                if (datePart) {
                    startIso = `${datePart}T${horaInicioClean}`;
                    endIso = `${datePart}T${horaFinClean}`;
                }

                const title = encodeURIComponent(`Partido en ${turno.complejo}`);
                const details = encodeURIComponent(`Reserva en ${turno.complejo}\nCancha: ${turno.cancha}\nDeporte: ${turno.deporte}\nMedio de Pago: ${turno.medioPago}\nTotal: $${Number(turno.precio).toLocaleString('es-AR')}`);
                const location = encodeURIComponent(turno.direccion || turno.complejo);
                ...
                this.showToast('No se pudo abrir el calendario', 'error');
            }
        },

        abrirComoLlegar(turno) {
            this.showMenuId = null;
            const destino = turno.direccion ? `${turno.complejo}, ${turno.direccion}` : turno.complejo;
            const mapsUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(destino)}`;
        ...
        abrirTelefono(turno) {
            ...
            if (turno.telefono && turno.telefono.trim()) {
                window.open(`tel:${turno.telefono.trim()}`, '_self');
            } else {
                this.showToast('El complejo no tiene teléfono registrado', 'info');
            }
        }
    }
}
