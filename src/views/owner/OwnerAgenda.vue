<template>
  <div class="min-h-screen bg-[#F8FAFC] pb-2 text-slate-800" style="font-family: Inter, 'Noto Sans', sans-serif;">
    ...
    <!-- HEADER -->
    <header class="sticky top-0 z-50 bg-white shadow-sm border-b border-slate-100 px-4 py-4">
      <div class="flex items-center justify-between mb-4">
        <button @click="$router.back()" class="w-10 h-10 flex items-center justify-center bg-slate-50 rounded-xl text-slate-400 active:scale-90 transition-all">
          <i class="fas fa-chevron-left text-sm"></i>
        </button>
        <div class="text-center">
          <h1 class="text-sm font-black uppercase tracking-widest text-slate-400 mb-0.5">Agenda</h1>
          <p class="text-base font-bold text-slate-900 leading-tight">{{ canchaNombre }}</p>
        </div>
        <button class="w-10 h-10 flex items-center justify-center bg-blue-50 rounded-xl text-[#2D9CDB] active:scale-90 transition-all">
          <i class="fas fa-ellipsis-v text-xs"></i>
        </button>
      </div>

      <!-- DATE SELECTOR -->
      <div class="flex items-center justify-between bg-slate-50 rounded-2xl p-2">
        <button @click="changeDate(-1)" class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-slate-900">
          <i class="fas fa-chevron-left text-xs"></i>
        </button>
        <div class="flex flex-col items-center">
          <span class="text-[10px] font-black text-[#2D9CDB] uppercase tracking-tighter">{{ displayMonth }}</span>
          <span class="text-sm font-bold text-slate-900">{{ displayDay }}</span>
        </div>
        <button @click="changeDate(1)" class="w-10 h-10 flex items-center justify-center text-slate-400 hover:text-slate-900">
    ...
    <!-- FLOATING ACTION BUTTON (Optional helper) -->
    <div class="fixed bottom-8 right-6">
       <button class="w-14 h-14 bg-slate-900 text-white rounded-2xl shadow-2xl flex items-center justify-center active:scale-95 transition-all">
    ...
    <transition name="fade">
      <div v-if="selectedTurn" class="fixed inset-0 z-[2000] flex items-end justify-center p-4">
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" @click="selectedTurn = null"></div>
        <div class="relative w-full bg-white rounded-[32px] p-8 animate-slide-up shadow-2xl">
          <div class="w-12 h-1.5 bg-slate-100 rounded-full mx-auto mb-6"></div>
          
          <div class="flex items-center gap-4 mb-8">
            <div class="w-16 h-16 rounded-2xl flex items-center justify-center text-2xl" :class="selectedTurn.status === 'reserved' ? 'bg-blue-50 text-[#2D9CDB]' : 'bg-emerald-50 text-emerald-500'">
               <i :class="selectedTurn.status === 'reserved' ? 'fas fa-user-check' : 'fas fa-calendar-plus'"></i>
            </div>
            <div>
              <h3 class="text-xl font-bold text-slate-900">{{ selectedTurn.status === 'reserved' ? selectedTurn.user : 'Espacio Libre' }}</h3>
              <p class="text-sm text-slate-400">{{ displayDay }} • {{ selectedTurn.start.split(' ')[1] }} hs</p>
            </div>
          </div>
          ...
          <div class="space-y-3">
            <button class="w-full py-4 bg-[#2D9CDB] text-white font-bold rounded-2xl shadow-xl shadow-blue-100 active:scale-95 transition-all text-sm uppercase tracking-widest">
              {{ selectedTurn.status === 'reserved' ? 'Ver detalle reserva' : 'Reservar manualmente' }}
            </button>
            <button class="w-full py-4 bg-red-50 text-red-500 font-bold rounded-2xl active:scale-95 transition-all text-sm uppercase tracking-widest">
